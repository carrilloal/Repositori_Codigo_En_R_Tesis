---
title: "Analisi de investigacion"
...
## Introduccion
En este documento presentamos una parte del analisis realizado a nuestra tesis de la carrera de ingenieria en sistemas de la UNAH. Para ello se octuvo informacion mediante una encuesta realizada en Google Forms de la cual se registraron un total de 130 respuesta, la encuesta estaba conformada con un conjunto de preguntas las cuales nos gereran un total de `r  length(survey) ` variables de las cuales solo trabajaremos algunas de ellas para mostrar algunos de los analisis realizados para nuestra investigacion. 

Este documento esta sub dividido en las siguientes secciones: 

1. nuestras variables.
2. Limpieza de datos.
3. Analisis descriptivo.
4. Analisis correlacional.
5. Analisis explicativo.
6. Solucion tecnologica.


### variables de nuestra encuesta
```{r  }
...
```
Observamos la siguiente tabla que muestra las variables que contiene valores NA. Aquellos valores maypres a cero son los que contiene valores de este tipo.
...
en la seccion de nalisis descriptivo solo se mostrara el analisis para algunas de las varaiables que seran utilizadas posteriormente en el analisis de correlaciones.


#### Variable Rango_academico

los resultados nos indican que la mayoria de estudiantes se encuentra en un rano de 70 a 79 el porcentaje que presenta es de un 45%, otro porcentaje inportante es de los estudiantes que se encuentran en un rango academico de 80 a 89 con un indice de 35%
``` {r , echo=FALSE}
...
mediante la funcion boxplot podemos darnos cuenta de que no tenemos valores atipicos
``` {r, echo=FALSE }
...
#### Variable Excelencia_academica

los resultados de esta variable nos indica que la matyoria de alumnos a pertenecido o pertenece a los estudiantes que se destacan con excelencia academica.
``` {r , echo=FALSE}
...
mediante la funcion boxplot podemos darnos cuenta de que no tenemos valores atipicos para esta variable ya que es una pregunta cuya respuestas son si y no
...
tambien lo podemos ver reflejado mediante un histograma para la distribucion de nuestra vairable.
``` {r }
...
los resultados de esta variable nos indica que la mayoria de alumnos cumple con sus deberes como estudiante presentando una disciplina de estudio.
``` {r , echo=FALSE}
...
mediante la funcion boxplot podemos darnos cuenta de que no tenemos valores atipicos y que la mayoria de respuestas se encuentra en las categorias de si cumple con sus deberes y en ocasiones 
...
mediante la funcion qqnorm reflejamos que tenemos unos valores que estan separados de los demas pero aun asi no presenta problemas de valores atipicos
``` {r }
...
#### Variable Autodidacta


los resultados de esta variable nos indica que la gran mayoria de estudiantes son autodidactas, esto quiere decir que tienen la facilidad o el compromiso de aprender por sus propios medios con un indice de 90.7%.
``` {r , echo=FALSE}
...
mediante la funcion boxplot podemos darnos cuenta de que no tenemos valores atipicos y que la mayoria de respuestas se encuentra en que si son autodidactas
...
tambien lo podemos ver reflejado mediante un histograma para la distribucion de nuestra vairable que muestra que es normal.
``` {r }
...
1. Rango_academico vs Excelenci_academica.
2. Disciplina vs Rango_academico.
3. Autodidacta vs Rango_academico


#### Rango_academico vs Excelencia_academica


Los resultados reflejan que la mayor cantidad de estudiantes  no se encuentra en una excelencia académica, pero que un buen numero de ellos si esta en condiciones de excelencia academica.
```{r }
table(survey$Rango_.acad.mico,survey$Exelencia_academica)
...
Mediante el siguiente grafico podemos apreciar mejor estos resultados, pero tambien el grafico nos indica que estos estudiantes que no estan en excelencia academica pues en alguna vez si lo han estado.
...
H_O Las categorias Rango academico y Excelencia academica son Independientes

H_A Las categorias son Dependientes

Conclusion: segun nuestro p-value con un valor de 8.58e-06 que es un valor menor a 0.05 aceptamos nuestra hipotesis nula por lo tanto nuestras variables son independientes.


#### Disciplina vs Rango_academico 

Los resultados para la variable disciplina respecto al rango académico indican que aquellos que cumplen con sus deberes de estudio óseo que tienen una disciplina se encuentran dentro de los rangos más altos cabe destacar que la mayoría de los que indicaron que si tienen una disciplina no pertenecen a una excelencia académica, pero se encuentran muy cerca además de eso es bastante el número que indica que si tiene una disciplina y que si se han destacado en una excelencia académica.

```{r }
table(survey$Disciplina,survey$Rango_.acad.mico)

prop.table(table(survey$Disciplina,survey$Rango_.acad.mico),1)

```


Estos resultados se ven mejormente reflejados en el siguiente gráfico que corrobora lo explicado anteriormente.
...
Respecto a la dependencia o independencia de las variables según chi2 se obtuvieron los siguientes resultados.
...
H_O  las categorías disciplina y rango_academico son independientes.

H_A las categorías son dependientes. 

Conclusión: según nuestro p_value no es menor a 0.05 entonces rechazamos nuestra hipótesis nula, por lo tanto, nuestras variables son dependientes.


#### Autodidacta vs Rango_academico

Para la variable autodidacta vemos resultados muy parecidos a los de disciplina vemos que aquellos estudiantes que si son auto didactas se encuentran en rangos de índice elevados además podemos decir que es mucho mayor cantidad de estudiantes que si son autodidactas esto nos indica que la carrera de ingeniería en sistemas lo demanda.
```{r }
table(survey$Autodidacta,survey$Rango_.acad.mico)

prop.table(table(survey$Autodidacta,survey$Rango_.acad.mico),1)
...
H_O las categorías autodidacta y Rango_academico son independientes
...
### Correlación Autodidacta vs Uso_lms
...
Como Se puede apreciar en la gráfica aquellos estudiantes que no son autodidactas corren más riesgo de no mejorar su rendimiento mediante el uso de plataforma LMS, y es a este grupo de estudiantes al que se le tiene que dar un mayor seguimiento.


### Correlación Plataforma vs Uso_lms 
...
La relación con esta variable pues no hace mucho sentido debido que muestra que el riesgo de mejorar el rendimiento mediante el uso de plataforma LMS. Se esperaría que tengan mayor riesgo los que han indicado que no es de importancia el uso de plataformas virtuales para la educación de hoy en día.
